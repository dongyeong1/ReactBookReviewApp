...
const PostEditModal = ({editModal,setEditModal,post}) => {
  ...
  return (
    <div>
        {post&&<Modals style={customStyles}  isOpen={editModal} onRequestClose={handleCancel}>
    ...
    <ReviewWrapper>
        <div>
            <CardWrapper
                hoverable
                  ...
                  cover={<img alt="example" height='400' src={post.src} />}
            >
                <Card.Meta 
        ...
        <ContentWrapper >
            ...
            <FormItem label="평점" required>
                <Rate   Rate te placeholder={post.rate}  value={rate} defaultValue={post.rate}  onChange={onChangeRate}></Rate>
            </FormItem>
            <FormItem label="내용" required>
                <Input.TextArea className='textArea'   placeholder={post.content} value={content}  onChange={onChangeContent}></Input.TextArea>
            </FormItem>
